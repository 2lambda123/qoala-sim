...
def run_n_node_app_separate_inputs(
    ...
    for name in names:
        ...
        remote_pids = {remote_batch.batch_id: [p.pid for p in remote_batch.instances] for remote_batch in remote_batches}
        procnode.initialize_processes(remote_pids, linear=linear)
...
def run_n_node_app(
    ...
    return run_n_node_app_separate_inputs(
        num_iterations, programs, new_inputs, network, linear
    )
