...
class RequestRoutineParser:
    ...
    The metadata lines must be in the given order. callback_type and callback are optional. But if callback is specified,
    then callback_type must also be specified. Other lines are required. req_name, callback_name and variable names in
    return_vars must be valid variable names. Callback type must be one of the following: 'sequential', 'wait_all'
    (case-insensitive). virt_ids must follow the format given in its description. typ must be one of the following:
