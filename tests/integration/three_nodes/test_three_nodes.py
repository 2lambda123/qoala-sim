...
def run_three_nodes(num_iterations: int) -> Result:
    ...
    charlie_node_cfg = create_procnode_cfg("charlie", charlie_id, num_qubits, determ=True)
    ...
    for i in range(num_iterations):
        bin_pattern.append(EhiNetworkTimebin(nodes=frozenset([alice_id, bob_id]), pids={alice_id: i, bob_id: i}))
        bin_pattern.append(EhiNetworkTimebin(nodes=frozenset([alice_id, charlie_id]), pids={alice_id: i, charlie_id: i}))
        bin_pattern.append(EhiNetworkTimebin(nodes=frozenset([bob_id, charlie_id]), pids={bob_id: i, charlie_id: i}))
    network_schedule = EhiNetworkSchedule(
        bin_length=1000,
        first_bin=0,
    ...
    for node in network.nodes:
        network.nodes[node].network_ehi.network_schedule = network_schedule
...
def test_three_nodes():
    ...
    for i in range(num_iterations):
        assert result.alice_results.results[i].values["m_ab"] == result.bob_results.results[i].values["m_ba"]
        assert result.alice_results.results[i].values["m_ac"] == result.charlie_results.results[i].values["m_ca"]
        assert result.bob_results.results[i].values["m_bc"] == result.charlie_results.results[i].values["m_cb"]
